...
<head>
    ...
    <link rel="stylesheet" href="./public/css/wap_apply_friend_list.css?_version=<?php echo $versionCode?>" />
    <link rel="stylesheet" href="./public/css/hint.min.css?_version=<?php echo $versionCode?>">
    <link rel="stylesheet" href="./public/css/wap.css?_version=<?php echo $versionCode?>" />
...
<body>
<!--左列， 以及会话帧-->
<div style="background-color: red;height: 5rem;display: none;    overflow-y: scroll;" class="site-warning"></div>
<?php include(dirname(__DIR__) . '/base/wapBaseView.php'); ?>
<!-- 右边聊天窗口，包括消息帧-->
<div class="layout-right msg-chat-dialog" style="display: none;z-index: 999999">
    <div class="chat-dialog" style="display: none">
        <?php include(dirname(__DIR__) . '/base/wapChatDialog.php'); ?>
    </div>
...
<div class="layout-right friend-apply-dialog" style="display: none;">
    <?php include(dirname(__DIR__) . '/base/wapFriendApply.php'); ?>
</div>
...
<?php include(dirname(__DIR__) . '/base/wapTemplate.php'); ?>
<?php include(dirname(__DIR__) . '/base/wapTempMsg.php'); ?>
...
<script src="./public/js/im/webGroupMsg.js?_version=<?php echo $versionCode?>"></script>
...
<script type="text/javascript">
    ...
    function setHeight(){
        ...
        try{
            $(".friend-list-contact-row")[0].style.height = Number(window.screen.height- 374-40)+"px";
            $(".left-body-chatsession")[0].style.height = Number(window.screen.height- 146.2-40)+"px";
            $(".group-list-contact-row")[0].style.height = Number(window.screen.height- 294-40)+"px";
            $(".wrapper-mask")[0].style.width = Number(window.screen.width)+"px";
            $(".left-body")[0].style.height = Number(window.screen.height-70-40)+"px";

            var program_num = $("#program-row-list").children().length;
            $("#program_list")[0].style.height = Number(program_num*80)+"px";
        }catch (error) {
